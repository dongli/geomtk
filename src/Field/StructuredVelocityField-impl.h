...
calcDivergence(const TimeLevelIndex<2> &timeIdx) {
    switch (mesh->getDomain().getNumDim()) {
        case 2:
            if (mesh->getDomain().getAxisStartBndType(0) == PERIODIC &&
                mesh->getDomain().getAxisStartBndType(1) == PERIODIC) {
                if (v[0].getStaggerLocation() == Location::CENTER &&
                    ...
                    for (int j = mesh->js(GridType::FULL); j <= mesh->je(GridType::FULL); ++j) {
                        for (int i = mesh->is(GridType::FULL); i <= mesh->ie(GridType::FULL); ++i) {
                            ...
                            double dx = (mesh->getGridInterval(0, GridType::FULL, i-1)+
                                         mesh->getGridInterval(0, GridType::FULL, i));
                            double dudx = (u2-u1)/dx;
                            double v1 = v[1](timeIdx, i, j-1);
                            double v2 = v[1](timeIdx, i, j+1);
                            double dy = (mesh->getGridInterval(1, GridType::FULL, j-1)+
                    ...
                    for (int j = mesh->js(GridType::FULL); j <= mesh->je(GridType::FULL); ++j) {
                        for (int i = mesh->is(GridType::FULL); i <= mesh->ie(GridType::FULL); ++i) {
                            ...
                            double dx = mesh->getGridInterval(0, GridType::HALF, i);
                            double dudx = (u2-u1)/dx;
                            double v1 = v[1](timeIdx, i, j-1);
                            double v2 = v[1](timeIdx, i, j);
                            double dy = mesh->getGridInterval(1, GridType::HALF, j);
                            double dvdy = (v2-v1)/dy;
                            div(timeIdx, i, j) = dudx+dvdy;
                        }
                    }
                ...
                if (v[0].getStaggerLocation() == Location::CENTER &&
                    ...
                    for (int j = mesh->js(GridType::FULL); j <= mesh->je(GridType::FULL); ++j) {
                        for (int i = mesh->is(GridType::FULL); i <= mesh->ie(GridType::FULL); ++i) {
                            ...
                            double dx = (mesh->getGridInterval(0, GridType::HALF, i-1)+
                                         mesh->getGridInterval(0, GridType::HALF, i));
