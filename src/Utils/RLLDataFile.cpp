...
void RLLDataFile::inputHorizontalGrids() {
    ...
    for (int m = 0; m < 2; ++m) {
        ...
        CHECK_NC_GET_ATT(ret, fileName, domain.getAxisName(m), "units");
        if (m == 0) {
...
void RLLDataFile::inputVerticalGrids() {
    ...
    CHECK_NC_INQ_DIMLEN(ret, fileName, domain.getAxisName(2));
    if (domain.getVertCoord().getType() == CLASSIC_PRESSURE_SIGMA) {
        ...
        CHECK_NC_GET_VAR(ret, fileName, domain.getAxisName(2));
    ...
    } else if (domain.getVertCoord().getType() == HYBRID_PRESSURE_SIGMA) {
        REPORT_ERROR("Under construction!");
    } else if (domain.getVertCoord().getType() == PRESSURE) {
        vec full(len);
        ret = nc_get_var(fileID, fullVarIDs[2], full.memptr());
        CHECK_NC_GET_VAR(ret, fileName, domain.getAxisName(2));
        memset(units, 0, sizeof(units));
        ret = nc_get_att_text(fileID, fullVarIDs[2], "units", units);
        CHECK_NC_GET_ATT(ret, fileName, domain.getAxisName(2), "units");
...
void RLLDataFile::outputGrids() {
    ...
    for (int m = 0; m < domain.getNumDim(); ++m) {
        ...
        CHECK_NC_PUT_ATT(ret, fileName, domain.getAxisName(m), "units");
        ret = nc_put_att(fileID, halfVarIDs[m], "units", NC_CHAR,
                         units.length(), units.c_str());
        CHECK_NC_PUT_ATT(ret, fileName, domain.getAxisName(m)+"_bnds", "units");
    }
